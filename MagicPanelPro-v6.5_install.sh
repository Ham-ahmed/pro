#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/Ham-ahmed/pro/refs/heads/main/install.sh -O - | /bin/sh
...
# Download the plugin
if ! wget -q "--no-check-certificate" "${GITHUB_BASE}/MagicPanelPro_v6.5.tar.gz"; then
...
if ! tar -xzf "MagicPanelPro_v6.5.tar.gz"; then
